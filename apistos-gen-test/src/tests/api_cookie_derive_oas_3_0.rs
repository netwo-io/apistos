...
fn api_cookie_derive() {
  ...
  let header_parameter = <OrganizationSlugCookie as ApiComponent>::parameters(OpenApiVersion::OAS3_0, None);
...
fn api_cookie_derive_deprecated_attribute() {
  ...
  let header_parameter = <OrganizationSlugCookie2 as ApiComponent>::parameters(OpenApiVersion::OAS3_0, None);
  assert!(schema.is_some());
  assert!(child_schemas.is_empty());
  assert!(!header_parameter.is_empty());
  let (schema_name, schema) = schema.expect("schema should be defined");
