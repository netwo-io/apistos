...
fn api_operation() {
  ...
  let components = test::components(OpenApiVersion::OAS3_1);
  ...
  let operation = test::operation(OpenApiVersion::OAS3_1);
...
fn api_operation_impl_responder() {
  ...
  let components = test_async::components(OpenApiVersion::OAS3_1);
  // only one component here because: error does not have schema and Test is used both for query and response
  assert_eq!(components.len(), 1);
  let components = serde_json::to_value(components).expect("Unable to serialize as Json");

  let operation = test_async::operation(OpenApiVersion::OAS3_1);
  let operation = serde_json::to_value(operation).expect("Unable to serialize as Json");
...
fn api_operation_deprecated() {
  ...
  let components = test2::components(OpenApiVersion::OAS3_1);
  assert_eq!(components.len(), 1);
  let components = serde_json::to_value(components).expect("Unable to serialize as Json");

  let operation = test2::operation(OpenApiVersion::OAS3_1);
  let operation = serde_json::to_value(operation).expect("Unable to serialize as Json");
