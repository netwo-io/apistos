...
mod scope_module {
  ...
  #[get(path = "/test/guard", guard = "image_guard")]
  pub(crate) async fn guard() -> impl Responder {
    HttpResponse::Ok()
  }
  ...
  #[get(path = "/twice-test/{value}")]
  pub(crate) async fn twice(value: web::Path<String>) -> impl Responder {
  ...
  #[post(path = "/test")]
  pub(crate) async fn post() -> impl Responder {
    HttpResponse::Ok().body("post works")
  }

  #[delete(path = "/test")]
  pub(crate) async fn delete() -> impl Responder {
    "delete works"
  }
  ...
  pub(crate) async fn multiple_shared_path() -> impl Responder {
    HttpResponse::Ok().finish()
  }
  ...
  #[options(path = "/test3")]
  #[trace(path = "/test4")]
  pub(crate) async fn multiple_separate_paths() -> impl Responder {
    HttpResponse::Ok().finish()
  }

  // test calling this from other mod scope with scope attribute...
  pub(crate) fn mod_common(message: String) -> impl Responder {
...
async fn scope_v1_v2_async() {
  let srv = actix_test::start(|| App::new().service(mod_scope_v1::test).service(mod_scope_v2::test));
